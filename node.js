...
/* updating card */
function updateCardElement(product) {
    const cards = document.querySelectorAll('.card')
    for (const card of cards) {

        if (parseInt(card.dataset.id) === product.id) {
            card.querySelector('.rate').textContent = `rating ${product.rating}/10`
            card.querySelector('.reviw').textContent = `review: ${product.review}`;
